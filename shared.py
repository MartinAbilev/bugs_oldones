...
class NeuralNetwork:
    ...
    def sort(self):
        ...
        for n in self.neurons:
            ...
            if n.nid in n.lowest:
                n.lowest.remove(n.nid)
            n.lowest = sorted(set(n.lowest))

    def randomizeWeights(self, factor):
        for neuron in self.neurons:
            for con in neuron.conection:
                con.weight = con.weight + (random.randint(-1, 1) * factor)
                if con.weight > 1:
                    con.weight = 1
                if con.weight < -1:
                    con.weight = -1
    ...
    def loadNet(self):
        ...
        for neuron in neuronDatta:
            self.neurons.append(Neuron(i))
            self.neurons[i].nid = int(neuron[1])
            xy = neuron[2].split(',')
        ...
        for neuron in self.neurons:
            if neuron.nid >= i:
                for con in self.neurons:
                    if con.nid >= i:
                        neuron.conectInToOut(con.nid, 0, 1, 1)
